...
- **Compile-error recovery: the iframe never strands.** Three
  ...
  pre-flights before deciding what to publish. The canonical wiring
  is the **closure/route-aware** capability —
  `createCoordinator({ site, verifyReachableRoutes: { cache } })`
  (pass the *same* `Cache` as `createPreviewHandler` so the warmed
  closure makes the render a cache hit). It verifies the import
  ...
  **delete path is symmetric**: a delete that strands a still-
  ...
  delete nothing reachable imports prunes as before. The legacy
  single-file `compile` hook still works (superseded when
  `verifyReachableRoutes` is also set) but cannot see a missing
  import and leaves deletes unconditional. Clean results flow
  through the existing `update`/`prune` walk; failures publish an
  HMR `error` with structured `diagnostics` / `codeFrame` /
  `snippet` (projected by `projectCompileError`, now **exported**
  from `@astroflare/host-cloudflare` — one implementation, host
  never re-projects). (2) `createPreviewHandler` wraps every 5xx +
  404 in an HTML envelope that re-injects the HMR client `<script>`
  (gated by `hmr !== false`), so a manual reload onto a broken page
  still attaches a live socket and recovers on the next clean
  change. (3) The auto-injected client now bundles
  ...
  `SnapshotError`'s shape from PR #15). All backwards-compatible —
  `verifyCompile` (and the symmetric delete guard) is opt-in,
  callers passing no pre-flight keep historical behaviour including
  unconditional delete prune; error-response injection follows the
  existing `hmr !== false` gate; the structured `HmrError` fields
  are optional. Coverage:
  [`packages/host-cloudflare/src/coordinator.test.ts`](packages/host-cloudflare/src/coordinator.test.ts)
  (`verifyReachableRoutes` describe — clean/missing-import/syntax/
  recovery/orphan/delete-prunes/delete-strands).
