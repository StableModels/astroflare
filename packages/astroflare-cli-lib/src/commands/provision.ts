/**
 * `provisionFixture` — create the Cloudflare resources for a managed
 * Worker bundle and write the resulting state to
 * `tests/e2e/.state/<sha7>/<name>.json`.
 *
 * Resource set today: one Worker script + one R2 bucket per name.
 * Durable Object classes ride along inside the Worker bundle — no
 * separate API call is needed (DOs are class registrations, not
 * resources).
 *
 * Idempotent. Re-running on an already-provisioned name sees the
 * existing state file and short-circuits unless `force: true`.
 *
 * "Fixture" is historical; the function works on any named bundle —
 * test fixtures, ad-hoc debug deploys, scripted reproductions of a
...
export interface ProvisionInput {
	...
	workerBundle: string;
	/** Pattern: `aflare-<name>-<sha7>` by default. */
...
const DEFAULT_NAME_PATTERN = (fixture: string, sha7: string): string => `aflare-${fixture}-${sha7}`;
...
export async function provisionFixture(input: ProvisionInput): Promise<FixtureState> {
	...
	// workers.dev subdomain so callers can fetch the deployed Worker.
